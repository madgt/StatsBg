...
$url = "https://www.boardgamegeek.com/xmlapi2/plays?username=marcelodgt";
...
{ 
	...
	{ 
		...
		{ 
			...
			// PROCURAR SE JÁ EXISTE PARTIDA COM PlayId
			...
			if($oldPoints->num_rows > 0)
			{
				while($row = $oldPoints->fetch_assoc())
				{
					...
					$sqlOldPoints = "Update points set playId =".$playId.",playerId=".$playerBdId.
							      ",score='".$playerScore."',role = '".mysql_real_escape_string($playerRole). "', winner = ".$playerWin." where playId = ".$row['playId']." and playerID=".$playerBdId;
